...
## スキル名: `html`
概要: 回答・作業結果・実装説明を、人間が見る1枚もののHTMLに整える窓口。型はサンドイッチ構造（結論→先に質問→図解中心の説明→推奨つき再質問）の1本で、実装・変更の説明は3点セット＋「合意まで対象を編集しない」理解ループを内蔵する（2026-07-22にquick/full二択とmeta-explain workflowを統合・廃止）。
近接・注意: 表示専用の成果物を直接作るため `applied`。正本(md)・同期対象(デイリー/計画)は対象外。読者想定は「コーディングしないがGO/NGを判断する人」。軽いすり合わせは `naiyou-suriawase`、問いで詰める壁打ちは `grill-me`。
