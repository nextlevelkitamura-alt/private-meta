...
この手順は、大目標が達成されたとAIが判断した節目でのみ実行する。hookによる自動判定・注入は行わない。
状態の⏸への機械flipは `../../events/session-end/mark-wait.py` が `Stop` ごとに行う（この手順とは独立）。Claude/Codexとも同じ共通実行本体を呼ぶ。

`<board>` = `~/Private/personal-os/AIエージェント基盤/hooks-registry/shared/session-board/board.py`。
...
> ① 完了成果を board へ記録（finish・親＝タスク／子＝時刻付き成果の入れ子。focusmap の「終わったこと」に表示される）
...
1. `<board> finish --key <キー> --repo <repo> --parent <目標名> --entry <成果1行>`（`--entry` は数だけ繰り返し・親名=行の目標名にする）。
   自行（`sessions`）が board DB から消え、成果は `session_logs` に **親＋時刻・所要(+Nm)付き子** で記録され、
   focusmap の「終わったこと」に表示される
...
- 残作業を `<board> update --now`（目的が変われば `--goal`）で書き換えて続行（⏸への遷移は `mark-wait.py` が保証。ただし🔵サブ稼働中は維持され⏸にしない）。
