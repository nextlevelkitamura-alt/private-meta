...
承認後に追加する`guard-plan-closeout.py`は別handlerで、`PLAN_RUN_MANIFEST`がある時だけ計画同期忘れを検査する。session-boardの状態や計画本文は変更しない。`finish`はsession-board記録を閉じるだけで、archiveの承認・実行ではない。
