...
4. `知識/`: 完成した恒久・再利用可能な参照md。未確定の考え・調査や特定計画だけの資料は置かない（`identity.md`、計画の `方針`、`references/` に寄せる）。人間向けHTMLが必要なら同名で隣接する。
5. `plans/`: 実行する計画（バケット管理）。計画に紐づく人間向けHTMLは各計画の `explain/` に置く。

考え・調査・仮説は独立フォルダにせず、`identity.md`（判断軸）か、育成中の計画の `plans/planning/<計画>/plan.md` の `方針`（未確定のまま育てる）に寄せる。`thinking/` は廃止した。
