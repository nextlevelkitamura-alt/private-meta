...
1. 既定コックピット＝2ペイン: 右上=実装(claude sonnet5) / 右下=レビュー(codex gpt-5.5 xhigh)。これは `AIモデル一覧.md` の役割方針と `scripts/cockpit.sh` の既定に一致させる。左(計画+監督/claude)は任意の3ペイン目（`--pane` を3つ明示指定すれば `up` で起動できる）。使うかどうかの判断基準は `cockpit-supervisor` Skillを見る。
...
1. 構築: `scripts/cockpit.sh up --repo <name> --branch <english> --title "<計画名>"` を実行（既定＝実装=claude sonnet5／レビュー=codex gpt-5.5 xhighの2ペイン、左は空slot）。役割・モデルを変えるなら `--pane "役割:kind[:model[:effort]]"` を渡す（3ペインにする場合は計画/実装/レビューの3つを明示指定）。
...
2. `codex`: 既定2ペインではレビュー向け。既定 `gpt-5.5` / effort `xhigh`。`--model` / `--effort` で変更。
