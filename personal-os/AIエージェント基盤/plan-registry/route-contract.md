...
plan-registry の経路解決（triage決定手続き。`triage.md`）の二段ルーティングとhandoff出力の唯一の契約。計画本文、plan header parser、状態索引は所有しない。スキーマ識別子 `plan-triage.route/v1` は安定した契約名として据え置く（callerとfixtureが依存するため改名しない）。
