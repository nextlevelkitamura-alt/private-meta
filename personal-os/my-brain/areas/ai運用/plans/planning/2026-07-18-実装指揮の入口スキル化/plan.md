...
2. **hook注入文に1行参照**（案C併用）: common.py `_mirror()` の 種別=実装 かつ 計画あり分岐に、実装開始時はこのSkillを開く旨の1行を追加（人間ゲート）。あわせて、program.md子計画マップから「実装レビュー待ちの子が一括宣言の束ね数に達した」を決定的に検出できた時だけ「束ねレビューの時期」の1行をミラーへ出す拡張を検討する（機械判定のみ・レビュー実行や合否は強制しない。2026-07-18人間要望）
