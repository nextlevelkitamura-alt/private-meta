...
SessionStartフック（各 runtime の session-board-session-start.py 受け口）がこの手順を注入する。
行の登録自体は UserPromptSubmit フック（各 runtime の session-board-prompt-register.py 受け口）が最初のプロンプトで機械的に済ませる
