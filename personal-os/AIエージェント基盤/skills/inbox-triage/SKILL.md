---
name: inbox-triage
description: 依頼インボックスの指定された1行を、経路判定→規模判定(plan-triage)→plans/planningへの計画ドラフト起案→元行への結果印まで進める。起案までが仕事で、実装・実行・active化はしない。Use when 人間が「このインボックス行をトリアージして」とデイリーファイルと対象行を指定したとき。自動巡回は行わない。
---
...
対象行はクレーム印の無い生の行でよい。このSkillは渡された行を「対象行の現在の完全な文字列」として扱う。
**渡された1行だけを処理する。** インボックスの他の行を探索・処理しない。完全一致する兄弟行があっても触れない。
...
2. **雛形生成**: 先に出力先が手順1で決めた `plans/planning/` 配下の絶対パスであることを確認し、`~/Private/personal-os/AIエージェント基盤/skills/plan-ops/scripts/new-plan.sh --out <plan.mdの絶対パス> --class <分類> --kind <種別>` を使う。実引数契約は `../plan-ops/SKILL.md` §2.2が正本。scriptは親ディレクトリを作成し、既存ファイルを上書きしないが、planning配下かどうかは検査しないため、このSkillが実行前に担保する。フォルダ名は作成日（今日, `YYYY-MM-DD`）＋日本語企画名（15〜20字目安）。既存フォルダと衝突する場合は企画名を具体化する。
...
- **対象行が旧自動巡回の `→処理中(` で終わっている場合**: 互換処理として、その部分を丸ごと次に**置き換える**（追記しない）。
  ```
  →計画作成済み(<plan.mdの絶対パス>)
  ```
...
**現行の結果マーカー**: `→計画作成済み(` ／ `→サクッと判定(`。`→処理中(` ／ `→重複(` は旧自動巡回行の互換読取だけに使い、新規付与しない。

**進捗注記**: `→着手(担当X)` ／ `→完了` は、起案後に着手した指揮官が `→計画作成済み(<パス>)` の後ろへ追記する。`inbox-triage` 自身はこの2状態を書かない。

## 通知

**このSkillは通知を送らない。** osascript・Notion APIには触れず、起案結果を呼び出した人間へ返す。
