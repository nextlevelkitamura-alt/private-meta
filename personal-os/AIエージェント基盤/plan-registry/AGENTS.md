...
**計画は既定で「テーマから切り出した実行1本」の実行ライン方式（テンプレv3・直列化）**。未成熟な構想は計画にせず themes/（熟成層・`../../my-brain/areas/AGENTS.md` §1.2）で育て、実行を決めた時だけ planning へ切り出す。planning は「実行を決めた計画の順番待ち」に絞り、上限5（満杯時は themes/ へ差し戻す）。

**Program は例外へ格下げ（2026-07-24）**。並列 worktree での分離実装が本当に必要な時だけ選ぶ。programの子計画マップは `企画 / 計画 / 実装 / 評価 / 修正 / 人間確認 / 完了` を使う。実運用では `実装中`、`評価待ち`、`保留` の注記を許す。program化は、独立に卒業する子計画が2本以上必要になった時に選び、基本は6〜7子まで、OSレベルの大改修だけ10前後を目安にする。
...
- **セーブポイント評価に一本化（[SAVE]・2026-07-24）**。サクッとは実装者の自己確認、**ライト/フルとも [SAVE] 地点の評価1回・差し戻し上限1**（修正1回→再評価、再FAILのみ人間へ即エスカレーション）とする。従来のフル差し戻し上限2は1へ下げた。[SAVE] は既定で完了直前の1箇所、長い計画でも最大2箇所まで。
- 評価はまとめが既定。実行ラインの `[SAVE]` マーカー地点だけで採点し、工程ごとの都度評価は、後続工程がその成果物を直接使う（依存する）工程だけに限る。「毎計画に評価」ではなく「いいタイミングで終わった時のセーブポイントとして評価」する。
...
- **program の子ごと独立評価は廃止（2026-07-24）**。評価は親の [SAVE]（統合まとめ評価）に集約し、子は実装の全PASSで閉じて独立評価mdを持たない。親は統合まとめ評価の全PASSで完了する。人間確認は、AIだけでは解決できない認証または結果を変える未確定判断がある場合だけ使う。
