...
| `scripts/program-lint.sh` | 親子対応・backlink・状態語彙・完了条件を静的検査 | 内容の妥当性・評価合否・承認 | 読み取り専用 |
| `scripts/plan-lint.sh` | plan / 子計画 / program の実行契約、必須節、placeholder、並列レーン宣言を静的検査 | 計画内容の妥当性・評価合否・承認 | 読み取り専用 |
| `scripts/check-section.sh` | 指定節だけを表示・pattern照合 | 一致の意味と評価合否 | 読み取り専用 |
...
テンプレから生成されない `評価.md` と `修正.md` は、対象planの評価・修正文書を作る時に手でコピーして使う。いつ評価するか、誰が評価するか、何を完了とするかはこのSkillでは決めない。
