...
# (c) 種別=実装・計画=実値 → 催促行なし・レビュー宣言行のみ追加（2026-07-16有効化）
out = mirror_lines("実装", "実装評価修正ループ")
check("(c) 実装×実値 は2行+レビュー宣言行", len(out) == 3 and out[2].startswith("レビュー宣言を確認") and "finish≠archive承認" in out[2])
...
# (h) 2回目以降の計画ミラーも repo AGENTS が宣言した箱を案内する（+実値計画はレビュー宣言行つき）
out = mirror_lines("計画", "企画/01")
check("(h) 計画ミラーもrepo固有箱", len(out) == 4
