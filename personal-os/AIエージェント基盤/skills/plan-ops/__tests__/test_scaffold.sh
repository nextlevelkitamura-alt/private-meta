...
# ============================================================
# (a) new-plan.sh（単発plan.md・既定）: 目的/現状/方針/完了条件を満たす
# ============================================================
...
assert_contains "(b) 人間確認方針を持つ" "$(cat "$WORKDIR/b-program/program.md")" "人間確認方針: 最終一括"
assert_contains "(b) 子マップの並列欄を持つ" "$(cat "$WORKDIR/b-program/program.md")" "並列: <可/不可>"
assert_not_contains "(b) 子マップのレビュー欄を持たない" "$(cat "$WORKDIR/b-program/program.md")" "レビュー:"
assert_contains "(b) 子マップの人間ゲート欄を持つ" "$(cat "$WORKDIR/b-program/program.md")" "人間ゲート: <なし／実行前に個別承認が必要な操作>"

# ============================================================
# (c) new-child.sh: 目的/現状/方針/完了条件＋親計画backlinkが正しく解決する
# ============================================================
...
assert_not_contains "(c) レビュー欄を持たない" "$(cat "$WORKDIR/b-program/plans/11-新子.md")" "レビュー:"
assert_contains "(c) 人間ゲート欄を持つ" "$(cat "$WORKDIR/b-program/plans/11-新子.md")" "人間ゲート: <なし／実行前に個別承認が必要な操作>"
