...
mark_wait(){ python3 "$EV/session-end/mark-wait.py"; }
sync_subagent(){ python3 "$EV/subagent/sync-subagent-status.py"; }
...
echo "$J2_SS" | claude_start >/dev/null
OUT=$(echo "$J2_UP" | claude_prompt)
...
echo "$J4_SS" | claude_start >/dev/null
...
OUT=$(echo '{"session_id":"agent-xyz","cwd":"/tmp","hook_event_name":"SubagentStart"}' | sync_subagent)
