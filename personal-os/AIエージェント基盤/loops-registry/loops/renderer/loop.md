...
9. `notion-inbox-pull.sh` がNotion「依頼インボックス」DB（統合program N2・先）の「状態=新規」行を
   ローカル当日デイリーの「## 依頼インボックス」節末尾へ追記し、該当行を「回収済み」に更新する
   （ベストエフォート。トークン未設定/parent page未解決/API失敗/取り込み済みidの重複はいずれも
   警告のみでexit 0し、render.sh自体の成否には影響しない。取り込み済みidは
   `state/notion-inbox-pulled-ids` に記録し二重取り込みを防ぐ。詳細は `notion-inbox-pull.sh`
   冒頭コメント参照）。
