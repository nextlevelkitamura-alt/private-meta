...
縦の目標ladder（3年→年間→デイリーの的と履歴）は隣の `../ゴール/`。ここ（areas）は横の領域別。全体の今は当日デイリーの計画ボード区画（レンダラ自動描画。`../ダッシュボード.md` はポインタ）。
...
全領域の「今」は当日デイリー（`../ゴール/デイリー/<年>/<月>/<日>.md`）の自動3区画（今やっていること・待ち・計画ボード）で見る。基盤レンダラ（`AIエージェント基盤/loops-registry/loops/renderer/`）が active な計画（単発 `plan.md`・`program.md` 子計画マップ）を走査して自動描画するため、手動の索引更新は不要（2026-07-02 決定ログ#2-⑦実施）。

1. 優先マーク（◎/○）は各 plan.md / program.md 冒頭の `優先:` 行に書く（ボード行頭に表示される）。
2. 「次の一手」は plan.md / program.md 子計画マップの `次:` 行が正本。
3. ボードに載るのは active バケットの計画だけ（`paused`/`done`/`archive` は載らない。履歴は各 `plans/` とログ）。
4. `../ダッシュボード.md` は1行ポインタとして残置。中央ボードとしての手動更新は廃止。
...
3. 状態が変わったら `git mv` でバケット間を移す。
   - 新規 → `planning/`（すぐ実行する計画は `active/` 直行でよい）。着手 → `active/`。
   - 一時停止 → `paused/`。
...
1. 冒頭に `分類:`（skill/repo/loop/横断）、`種別:`（新規作成/既存改善/統合整理）。任意で `規模:`（フル/ライト。省略時フル）と `優先:`（◎/○）を足す。段階語彙・規模・人間ゲートの正本は `../../説明書/運用契約.md` §2。
...
2. **子の状態は program.md の子計画マップが持つ**（リスト形式: 「NN 子計画名 … 状態」＋ `次:` ＋ `場所:/依存:`。表は使わない）。子に状態バケットは作らない。マップの状態は運用契約§2の**段階語彙**（企画/計画/計画レビュー/実装/実装レビュー/修正/人間確認/完了。実運用語: 実装中/計画レビュー待ち/保留・括弧注記は可）で書く。§4.1の英語バケット語彙はフォルダ用で、マップには使わない（plan-ops `program-lint.sh` が機械チェックする。2026-07-08 決定ログ#12）。
...
テンプレ本文の正本は `../../AIエージェント基盤/skills/plan-ops/templates/`（`plan.md`・`program.md`・`子計画.md` の3枚。2026-07-02 子07で正本化）。本節にテンプレ本文を複製しない（契約§8）。雛形の生成は `skills/plan-ops/scripts/new-plan.sh` / `new-child.sh`（使い方は plan-ops SKILL.md）。

単発 plan.md は `目的/現状/方針/完了条件（レビュー項目）` の同一本文で、`形態: program` と子計画マップだけ持たない。
...
area の plan バケットは `planning/active/paused/done/archive`（§3）、基盤の plan バケットは6語フル（基盤 `AGENTS.md` §1.1）。
...
5. 計画ボードへの反映はレンダラが自動で行う（手動更新不要）。program の子なら program.md マップの「場所」を移行先に更新する。
...
3. 成熟した計画は §5 の卒業手順で実行repoへ移す。area には移行ログが残る（ボード表示はレンダラが自動追従）。
