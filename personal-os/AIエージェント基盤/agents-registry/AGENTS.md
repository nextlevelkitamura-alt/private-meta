...
2. runtimeへの露出は正本・用途・対象runtimeを確認してAIが行う。Claudeは `~/.claude/{commands|agents}/`、Codexは `~/.codex/agents/*.toml` またはproject `.codex/agents/*.toml` の現行仕様に従う。本文コピーは禁止。露出後にリンク先をreadbackする。
3. この AGENTS.md の登録一覧に1行追記する。
4. 削除は参照確認・一覧から除去・symlink撤去・回復手段の確認をセットで行う。対象が曖昧または回復不能な時だけ停止する。

## 置かないもの

- 他ツール由来の野良コマンド（`~/.claude/commands/` に残る旧実体ファイル群は legacy。移す時は正本・参照・runtime露出を確認する）。
