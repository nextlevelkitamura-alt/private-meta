#!/usr/bin/env python3
# session-board UserPromptSubmit hook（Codex受け口）
# 未登録なら board へ登録（要約=プロンプト先頭24字）。⏸なら🟢へ復帰。stdoutは常に空。
# subagent／headless／スラッシュコマンド／空・添付のみは対象外。契約は ../../references/codex-hooks.md。
import json
import os
import re
import subprocess
...
def main():
    ...
    # sub（🔵サブ稼働中）は触らない＝サブ完了までエージェント/Subagentフックが run へ戻す。
