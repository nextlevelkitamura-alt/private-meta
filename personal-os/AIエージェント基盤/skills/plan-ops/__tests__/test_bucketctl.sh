...
mkdir -p "$REPO/plans/planning/昇格対象" "$REPO/plans/planning/上限超過対象" \
  "$REPO/plans/active/実行中A" "$REPO/plans/active/実行中B" "$REPO/plans/active/実行中C" "$REPO/plans/active/実行中D"
...
assert_contains "(a) active件数の変化を表示" "$dry_out" "active: 4/5 → 5/5"
...
# (d) active=5を一箇所で強制し、既存超過はcheckで可視化する。
...
mkdir -p "$TEMP_PLANS/planning/$TEMP_NAME" "$TEMP_PLANS/planning/通常対象" \
  "$TEMP_PLANS/active/実行中A" "$TEMP_PLANS/active/実行中B" "$TEMP_PLANS/active/実行中C" "$TEMP_PLANS/active/実行中D" "$TEMP_PLANS/active/実行中E"
for p in "$TEMP_PLANS/planning/$TEMP_NAME" "$TEMP_PLANS/planning/通常対象" \
  "$TEMP_PLANS/active/実行中A" "$TEMP_PLANS/active/実行中B" "$TEMP_PLANS/active/実行中C" "$TEMP_PLANS/active/実行中D" "$TEMP_PLANS/active/実行中E"; do
  printf 'plan\n' > "$p/plan.md"
done
...
temp_limit_rc=$?
assert_eq "(d) active=5の流入は拒否" "$temp_limit_rc" "1"
assert_contains "(d) 上限と人間判断を表示" "$temp_limit_out" "上限5件"
check_out="$($SCRIPTS/bucketctl.sh check "$TEMP_PLANS" --json 2>&1)"
assert_contains "(d) check --jsonは対象一覧を返す" "$check_out" "実行中A"
assert_contains "(d) check --jsonは上限件数を返す" "$check_out" '"count": 5'
