...
`add`/`update`/`flip`/`log`/`finish`/`reconcile` は MD書き込み・flock解放後、ベストエフォートで Turso（`personal-os-board`・
...
- `add`/`update`/`flip` → `sessions` テーブルへ `session_key` で upsert（層1・機械送信・現在値のみ）。
...
- **状態遷移イベント**（層3・2026-07-11〜）→ `session_events` へ**追記式**で insert（上書きしない・時間集計の入力。
  ...
  `finish`＝done（行削除前のスナップショット）／`reconcile` の⏸降格＝wait（trig=reconcile。
  旧「reconcile は対象外」は撤回。`sessions` の上書きは従来どおりしない＝次の add/update/flip で追いつく）。
  `update`・`log`・`sub-start`/`sub-end` は打たない（Stop経路= flip wait→reconcile でも wait は1本だけ）。
  列: session_key／state(run|wait|sub|done)／at(ISO ms)／trig(add|flip|finish|reconcile・
...
- 送信は1コマンド1バッチに合流: sessions upsert・logs・delete・events を同一 `_turso_execute`
  パイプラインで送る（HTTP往復1回。reconcile はイベントのみ＝`turso_append_events`）。
...
- 認証: keychain `turso-personal-os-board`（`sessions`/`session_logs`/`session_events` を読み書きできる
  ...
  （`board.py` の `_turso_token()`／`_turso_execute()` を参照）。
- タイムアウト3秒・例外は握りつぶす（`_turso_execute`）。ネットワーク非依存のロジックのみテストで担保
  （イベント構築= `tests/test_events.py`・送信関数モック／集計SQL= `tests/test_events_sql.py`・in-memory sqlite3 に実DDL）。
