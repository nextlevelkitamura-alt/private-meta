...
#   ③ Orca CLI 応答確認 → 可なら cockpit.sh spawn で可視1ペインを起動（人が監督できる既定経路）。
#   ④ Orca 不可（または spawn 失敗）なら claude -p で headless 起動（フォールバック）。
...
if orca_responsive; then
  ...
  log "info: Orca 応答なし → headless で起動"
fi
