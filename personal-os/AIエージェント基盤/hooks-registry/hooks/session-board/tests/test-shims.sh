...
grep -qF "| ? | 今:? | repoZ | その他 | claude/? <!-- s:beefcafe -->" "$DAILY"; ok "枠行が登録される" test $? -eq 0
...
echo "=== 3. AIが行を正した後のUPS: 2行ミラー ==="
"$BOARD" update --key beefcafe --type 実装 --goal "ボード再設計" --now "common改修" --model fable5
OUT=$(echo "$J_UP" | python3 "$CL/prompt-register/session-board-prompt-register.py")
echo "$OUT" | grep -q "目標:ボード再設計 | 今:common改修 | 種別:実装"; ok "ミラー1行目" test $? -eq 0
...
echo "=== 5. 計画種別のミラーは置き場1行が付く(3行) ==="
"$BOARD" update --key beefcafe --type 計画
OUT=$(echo "$J_UP" | python3 "$CL/prompt-register/session-board-prompt-register.py")
echo "$OUT" | grep -q "計画の置き場"; ok "計画スニペット" test $? -eq 0
...
grep -qF "| codex/? <!-- s:c0dec0de -->" "$DAILY"; ok "Codex枠行(runtime=codex)" test $? -eq 0
