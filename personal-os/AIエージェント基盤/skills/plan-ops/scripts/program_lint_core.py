...
検査6種:
  ...
  6. 状態が「完了」の子について、その子ファイル自身の
     「## 完了条件」に未チェック（`- [ ]`）が残っていないか。
  7. programの親は `program.md` だけとし、`実装/共通.md` と `評価/` を必須、
     `plan.md` と `レビュー/` を禁止する。
  8. 親program・子planに旧 `レビュー:` フィールドを残さない。
"""
import glob
import os
import re
...
def lint(program_path):
    ...
    if not any("形態: program" in line for line in lines[:4]):
        out.append(f"{program_path}:1: programは先頭メタデータに「形態: program」が必要")
    sibling_plan = os.path.join(base_dir, "plan.md")
    if os.path.isfile(sibling_plan):
        out.append(f"{sibling_plan}:1: programの親はprogram.mdだけ。併存plan.mdは置けない")
    implementation_dir = os.path.join(base_dir, "実装")
    if not os.path.isfile(os.path.join(implementation_dir, "共通.md")):
        out.append(f"{program_path}:1: 実装/共通.md が無い")
    evaluation_dir = os.path.join(base_dir, "評価")
    if not os.path.isdir(evaluation_dir):
        out.append(f"{program_path}:1: 評価/ フォルダが無い")
    review_dir = os.path.join(base_dir, "レビュー")
    if os.path.exists(review_dir):
        out.append(f"{review_dir}:1: レビュー/ フォルダは使えない。評価/へ統一する")
    check_review_field(program_path, lines, out)
    ...
    for b in blocks:
        ...
        check_review_field(child_path, read_lines(child_path), out)
        if base == "完了":
