...
- [x] create-new.md / migrate-skill.md に段階露出の手順と未完了露出の追跡方法が記載され、kickoff / morning-routine の残り露出（~/.agents・~/.codex・~/.gemini系）がバックログとして列挙されている。
- [x] logs/AGENTS.md §6 の削除テンプレが実ログのフィールド（承認・吸収先）を含み、日付時刻に HH:mm を強制する記載になっている。既存ログの書き換えが無い。
- [x] skill-creator-custom と skill-creator-codex の SKILL.md が相互ポインタを持つ。
- [x] global-scan.md に三点照合ステップが1項ある。
- [x] 変更対象が skill-creator-custom／skill-creator-codex／logs/AGENTS.md／catalog に閉じている（doc only・スクリプト変更なし）。

※ 2026-07-21クローズ儀式でチェックボックス書式へ正規化（文言は不変）。採点は評価01.md（遡及）。

## 結果（2026-07-03・完了）

実行=中間指揮官1（采配10本目）・基盤commit e35434d（doc only 10ファイル）・codex 1パスレビューPASS（指摘0・差し戻しなし）・A検収済み。反映先=skills/skill-creator-custom（workflows4本）・skills/skill-creator-codex（相互ポインタ）・global-skill-registry/logs/AGENTS.md（未露出バックログ書式+削除テンプレ強化）・createdログ2件（残り4runtime露出をバックログ列挙）・catalog meta.md。push=基盤の一括push（spawnレビュー合流）に同乗。
