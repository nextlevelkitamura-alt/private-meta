...
6. ローカルへ保存する場合は `GLOBAL_AGENTS.md` §3–4を正本として、areaでは計画ごとの `explain/` と恒久知識の隣接HTMLを使い分ける。
7. ページ全体を白背景のライト単色にし、カード・図・コードブロックに暗い背景を使わない。表示環境による配色切替も作らない。

## 4. 確認して提示する

1. PCで白背景・暗い背景なし・表示環境による配色切替なしを確認し、表示とレイアウト崩れも確認する。
