...
| `pre-tool-use/`（承認後） | `guard-plan-bucket-move.py` | 計画バケットへの生移動をdeny |
| `session-end/` | `mark-wait.py` | 🟢を⏸へ更新 |
| `session-end/`（承認後・別handler） | `guard-plan-closeout.py` | manifestの未同期を一回だけ継続要求 |
| `subagent/` | `sync-subagent-status.py` | 🔵と稼働数を同期 |
| `subagent/`（承認後・別handler） | `verify-plan-worker.py` | 割当・result/evaluationを検査 |
