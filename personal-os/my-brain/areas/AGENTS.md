...
1. バケットが計画の状態の正本。意味は次の通り（area 内計画の状態）。
  ...
  - `active`: 今週着手・進行中の実行計画のみ。1 area あたり最大3件。
   - `paused`: 一時停止。再開予定あり。
...
3. 状態が変わったら `git mv` でバケット間を移す。
   ...
   - 一時停止、未完の軽微作業、保留 → `paused/`。理由と再開条件は各計画に残す。WIP整理のように同一理由で複数計画を一括移動する時だけ、対象計画を特定した決定ログ1件に理由と再開条件を残せばよい。
   - area内実行 完了（未評価）→ plan.md に結果を追記し `done/` へ。
   - 評価OK → `archive/`。問題があれば `active/` へ戻す。完了・評価済み以外を archive に入れない。
...
1. 冒頭に `分類:`（skill/repo/loop/横断）、`種別:`（新規作成/既存改善/統合整理）。任意で `規模:`（フル/ライト。省略時フル）と `優先:`（◎/○）を足す。段階語彙・規模・人間ゲートの正本は `../../AIエージェント基盤/GLOBAL_AGENTS.md` §7。
...
1. **実行ゲート**: レビュー項目が定義され着手可能になったら実行に出す。area計画は指揮官がペインへ配って実行する（§4.2）。基盤計画は `planning → ready` に上げる（loop が拾う印）。
...
2. **ラウンド番号＝差し戻し回数**。上限は `GLOBAL_AGENTS.md` §7（ライト1・フル2）。上限超過は人間へエスカレーション。
3. **書き手**: 採点はレビュー担当（実装と異系統のモデル）が行い、評価md・修正mdのファイル作成は指揮官（メイン）が行う。レビュー担当は自分でコードを直さない。
4. **doneゲートとの接続**: 最終ラウンドの評価mdが全PASSであることを done 移動の条件にする（§3「レビュー項目と実行ゲート」2の具体化）。サクッと（構造3条件全YES）は評価・修正mdを作らない。
5. 実装担当への差し戻しは「修正NN.mdを読め」で渡す（resume等の引数に要約を書いて済ませない）。
...
2. **子の状態は program.md の子計画マップが持つ**（リスト形式: 「`- [ ] NN 子計画名 … 状態`」＋ `並列:` ＋ `レビュー:` ＋ `次:` ＋ `場所:/依存:`。表は使わない）。見出し左のチェックボックスは完了だけ `[x]`、それ以外は `[ ]` とし、plan-opsが状態との不整合を検出する。`並列:` は他子と同時着手できるかを `可/不可` で、`レビュー:` は `GLOBAL_AGENTS.md` §7の判定に従い `都度/一括` で書く。子に状態バケットは作らない。マップの状態は `GLOBAL_AGENTS.md` §7の**段階語彙**（企画/計画/計画レビュー/実装/実装レビュー/修正/人間確認/完了。実運用語: 実装中/計画レビュー待ち/保留・括弧注記は可）で書く。§4.1の英語バケット語彙はフォルダ用で、マップには使わない（plan-ops `program-lint.sh` が機械チェックする。2026-07-08 決定ログ#12）。
...
テンプレ本文の正本は `../../AIエージェント基盤/skills/plan-ops/templates/`（`plan.md`・`program.md`・`子計画.md`・`評価.md`・`修正.md` の5枚。2026-07-02 子07で正本化・2026-07-11 評価/修正を追加）。本節にテンプレ本文を複製しない（正本変更の追従は `GLOBAL_AGENTS.md` §4・§7）。雛形の生成は `skills/plan-ops/scripts/new-plan.sh` / `new-child.sh`（使い方は plan-ops SKILL.md）。

単発 plan.md は `並列:`／`レビュー:` と `目的/現状/方針/完了条件（レビュー項目）` を持ち、`形態: program` と子計画マップだけ持たない。子計画.md も親計画backlinkに加えて `並列:`／`レビュー:` を持つ。
...
3. `active`: 今週着手・進行中の実行計画のみ（1 area あたり最大3件）。
4. `paused`: 一時停止。
5. `done`: 完了・未評価。
...
2. その計画から派生する実行作業は、指揮官がペインへ配って実行する（§4.2）。独立に卒業する子計画を2本以上生むなら program 化する（§3）。
