...
> [!NOTE]
> **本skillは `plan-registry` へ統合移設済み・物理削除は人間承認待ち（2026-07-22）。**
> 決定手続き（規模・二段ルーティング・fail-closed）と検証テストは `../../plan-registry/`（`AGENTS.md` §6 経路解決／`triage.md`・`route-contract.md`・`examples/`・`scripts/`）が正本。参照は全て plan-registry を指すよう更新済み。`skills/plan-triage/` の物理削除（`skill-delete` 経由・catalog行の除去・deletedログ）は人間ゲートのため未実行。この配下は削除承認までの残置で、正本として編集しない。
