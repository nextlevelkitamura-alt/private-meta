...
1. `<board> finish --key <キー> --repo <repo> --parent <タスク名> --entry <成果1行>`（`--entry` は数だけ繰り返し）。
   自行がボードから消え、成果が「終わったこと」の該当repo見出しに **親＋時刻付き子** で入る
   （途中で `log` 済みの子があれば、finish は同じ親にさらに子を足す・親は重複しない）。
...
- 残作業を `<board> update` で書き換えて続行（⏸への遷移は session-end.py が保証。ただし🔵サブ稼働中は維持され⏸にしない）。
