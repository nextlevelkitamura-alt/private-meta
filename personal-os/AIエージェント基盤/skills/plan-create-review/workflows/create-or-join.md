...
- 入力: やりたいこと1行、起点pathまたは起点種別、対象repo見当、影響範囲・戻しやすさ。分からない時の質問は `plan-registry` の経路解決（triage決定手続き §6）の上限に従う。
...
2. `plan-registry` の経路解決（triage決定手続き §6・`triage.md`）に従い、書込みなしで既存plan・計画箱・handoff要否を解決する。routeのcriteria、JSON、pathをここで再実装しない。
...
- 失敗時: path未宣言、候補競合、正本不明、handoff不正は経路解決（triage決定手続き §6）のstop結果を維持し、root `plans/` を推測・作成しない。
