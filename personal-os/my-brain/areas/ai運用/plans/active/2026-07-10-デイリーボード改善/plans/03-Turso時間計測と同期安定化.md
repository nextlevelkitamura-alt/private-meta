...
- [ ] 未確定4点が確定し、人間確認を経て本mdに追記されている。
- [ ] 状態遷移のたびに `session_events` へ1行追記され、mdには時間情報が増えていない。
- [ ] 実行時間・待ち時間・「⏸15分超」一覧がSQLで取得できる（サンプルクエリを本mdに記載）。
- [ ] reconcileの⏸降格がTursoへ反映される。
- [ ] secret/トークン値がコード・ログ・commitに現れない。
- [ ] 既存テスト全緑＋Turso失敗時にmd運用が一切阻害されない（ベストエフォート維持）。
