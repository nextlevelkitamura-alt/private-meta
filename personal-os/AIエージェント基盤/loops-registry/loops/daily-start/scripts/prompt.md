...
2. 手順の概略（詳細はスキル本文が正本・食い違ったらスキル本文を優先）:
   ...
   - 今日の大課題（themes **3〜5個・目安4個**）を確定し、不足分だけ `board.py theme-add` する（重複作成しない）。粒度は「今日はこれをします」と言い切れる塊。**1〜2個の粗いまとめにしない**（材料が本当に無い日だけ少なくてよい・その理由を1行残す）。
   - 今日やること（todos）を `board.py todo-add --theme <theme_id>` で各テーマの子として確定起票する（**必ず `--theme` で親テーマへ紐付け**・テーマに紐付かない todo は原則作らず突発は `ask` で人間に確認・各テーマに最低1件・計画対応は `--note` に計画パス・assignee/route を自己判定。**route='routine' の自称起票は禁止**＝宣言照合を経たものだけ）。繰越しは新todo起票＋`--carried-from <昨日>` で引き寄せる（繰越しも `--theme` 紐付け）。
