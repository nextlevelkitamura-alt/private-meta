...
縦の目標ladder（3年→年間→デイリーの的と履歴）は隣の `../ゴール/`。ここ（areas）は横の領域別。全体の今（動いているエージェント／終わったこと）は board DB（Turso）が正本で focusmap がDBから描画する（2026-07-21 正本反転・案b＝デイリーmd 2節は廃止）。
...
- **ai運用**は `identity.md` の役割を `AGENTS.md` へ統合済みで、2026-07-25にTheme専用構造へ移行した。ai運用直下の `plans/` と `知識/` は廃止し、必要な研究は各Themeの `concepts/research/` に置く。詳細は `ai運用/AGENTS.md` を正とする。
...
### 1.2 themes/（熟成層・目的を育てる常設場所）

**テーマは計画ではない**。目的（なぜ・どこまで）を育て、実行を決めた時だけ同じThemeの計画箱へ切り出す。Theme固有の詳細構造は最寄り `AGENTS.md` を正とする。

**ai運用のTheme専用構造（2026-07-25）**:
...
- ai運用ではTheme直下の `goal.md` が目的・現在の決定の正本である。構想、調査、会話記録は `concepts/` の固定3区分に置く。
- Theme固有の実行計画は `plans/<bucket>/<計画名>/plan-0.md` を親正本とし、評価は `evaluations/` に置く。独立した子だけ `plan-NN-<名前>/plan.md` を作る。
- ai運用直下の計画箱、知識フォルダ、決定ログは作らない。Themeに属さない実装計画は所有repoの最寄り `AGENTS.md` に従う。
- Markdown本文はローカル/Gitが正本であり、将来Tursoへ短い状態・活動履歴を移す候補でも本文を二重保存しない。

## 2. 全体俯瞰: 当日デイリーとactive計画

動いているAIセッション（動いているエージェント／終わったこと）は board DB（Turso）が正本で、focusmap がDBから描画する（2026-07-21 正本反転・案b＝デイリーmd 2節は廃止）。全area横断で「今activeな計画」を自動描画する仕組みは無い（旧レンダラは2026-07-04廃止・未再建。旧`../ダッシュボード.md`も同日撤去先を失い2026-07-08削除）。active計画を俯瞰するには各area `plans/active/` を確認するか、program.md子計画マップを読む。
...
計画本文の正本は常にmd（git）。計画本文のDB化は一方向の表示キャッシュに限る（DB→md書き戻しの経路を作らない）。
...
1. バケットが計画の状態の正本。意味は次の通り（area 内計画の状態）。
   - `planning`: 実行を決めた計画の順番待ち（未着手。上限5・2026-07-24）。未成熟な構想・アイデア・揺れている目的は planning でなく **themes/（熟成層・§1.2）** で育てる。
   - `active`: 実装・修正・評価中の計画。各 `plans/` root あたり最大5件。
   - `paused`: 一時停止。再開予定あり。
...
3. 状態が変わったら `git mv` でバケット間を移す。
   ...
   - 終了区分は `completed`／`superseded`／`merged`／`conflict`／`cancelled`。`completed` は全完了条件・最終評価全PASS（Programは全子完了）を要し、その他は理由・人間確認・未完了事項、さらに `superseded/merged/conflict` は後継・統合先を要する。移動は `bucketctl` を使い、上限は planning=5・active=6・paused=3・done=8（archiveは無制限）とする（2026-07-19人間承認でactive 4→5、2026-07-22人間承認で5→6、2026-07-24 themes/新設に伴い planning 無制限→5）。planning 満杯時は `bucketctl` が拒否し、未成熟な構想は themes/ へ差し戻す（§1.2）。
...
**実行ライン方式（テンプレv3）が既定（2026-07-24）**。計画は「テーマから切り出した実行1本」で、**基本は直列、並列は区間として埋め込む**。子計画への分割は既定で行わない（program化は例外・後述）。`new-plan.sh` は既定で v3 を生成し、重量版が要る時だけ `--legacy-v2` を使う。

1. 冒頭に `分類:`（skill/repo/loop/横断）、`種別:`（新規作成/既存改善/統合整理）、`テンプレ: v3`、任意で `規模:`（フル/ライト。省略時ライト）・`優先:`（◎/○）・`テーマ:`（§1.2 の theme.md への相対リンク）。規模、段階語彙、評価、人間ゲートの正本入口は `../../AIエージェント基盤/plan-registry/AGENTS.md`（runtime最小ゲートは同階層の `GLOBAL_AGENTS.md` §7）。
2. **v3 の必須セクション**: `目的`（テーマから1行引用）/ `完了条件`（テーマのゴールのうち今回満たすもの）/ `実行ライン`。`非対象/現状/実行契約/方針` は任意で、必要な内容だけを実行ラインの各ステップと `記録` 節に吸収する（**計画の重さの既定値を下げる**）。
3. **実行ライン**: `- [ ] 01 <ステップ>` の直列。並列は区間で埋め込む（`03 ⇉ 並列区間 { 03a … / 03b … } → 04で合流`）。セーブポイント評価は `[SAVE]` を付ける（1回・plan-registry §2）。`## 工程`（旧v2）と `## 実行ライン`（v3）は移行期の別名として daily-start / fetch-context.sh が両方読む。
4. **v2（legacy・重量版）** は `--legacy-v2` で生成。必須は `目的/非対象/現状/実行契約/方針/完了条件`＋実行契約フィールドで、`## 工程` 節を持つ。既存の v2/legacy 計画はそのまま有効（plan-lint はテンプレ版で検査を切り替える）。
5. `完了条件` は検証可能な**受入条件**で書く（下の「完了条件と実行ゲート」）。未確定なら「未確定」と明記。
6. 「背景: 未記入」のような空欄テンプレは禁止。書けない欄は消すか「未確定」と書く。

### 完了条件と実行ゲート

計画は検証可能な受入条件として完了条件を実行前に定義する。評価の規模別方式・人間ゲート・差し戻し上限は `../../AIエージェント基盤/plan-registry/AGENTS.md` を正とし、ここはareaでの文書配置と状態遷移だけを持つ。
...
2. **done ゲート**: 実行後、評価で全完了条件がPASSなら done。満たさなければ差し戻す。
3. 完了条件は「やったか」でなく「**こうなっていれば正しい**」を書く（例: 「テーブル行が残っていない」「リンクが全て解決する」「secret混入なし」）。
4. 各項目は**対象（ファイル/セクション）を明示**する（例: 「3年計画.md の領域別4行すべてにヒントが付く」）。範囲を絞らないと機械チェックが誤判定する。

### 評価・修正文書（実装評価サイクルのMD駆動）

ライト以上の実装は、採点結果と差し戻し指示もMD文書にする（口頭・チャット要約で渡すと指示が丸まって劣化するため）。書式の雛形は plan-ops `templates/評価.md`・`templates/修正.md`（本文をここに複製しない）。

1. **命名と置き場（接尾辞方式・2026-07-17改定）**: 単発は plan.md と同フォルダに `評価01.md` → `修正01.md` → `評価02.md` …。program は `<program>/評価/` へ集約し、子の分は `評価/NN-子名-評価01.md` のように子と同じ接頭NNを付け、program自身の統合評価は `評価/評価01.md` とする（`plans/` には子計画だけが並ぶ）。2026-07-17より前の旧配置（`plans/` 内の隣接・直下）は**読み取り互換**で、既存ファイルは移動しない。
2. **ラウンド番号＝差し戻し回数**。上限と書き手の責務は `../../AIエージェント基盤/plan-registry/AGENTS.md` に従い、上限超過は人間へエスカレーションする。
3. **doneゲートとの接続**: 最終ラウンドの評価mdが全PASSであることを done 移動の条件にする（§3「完了条件と実行ゲート」2の具体化）。サクッと（判定はplan-registry）では評価・修正mdを作らない。
4. 実装担当への差し戻しは「修正NN.mdを読め」で渡す（resume等の引数に要約を書いて済ませない）。

### プログラム計画（親子層）

**Program は例外（2026-07-24 格下げ）**。既定は実行ライン方式の単発 plan.md で、並列 worktree での分離実装が本当に必要な時だけ program を被せる。その場合も評価は子ごとに行わず親の `[SAVE]` に集約する（plan-registry §2）。

program化の判定と子計画の評価方式は `../../AIエージェント基盤/plan-registry/AGENTS.md` を正とする。ここではprogramの物理構成だけを定める。

```text
plans/active/<YYYY-MM-DD-日本語企画名>/   # programフォルダ（自身の状態はこのareaバケット）
  program.md            # 親＝索引（目的・全体像・子計画マップ・完了条件）
  plans/                # 子計画群（平置きファイル・子計画だけを置く）
    NN-<子計画名>.md     # 子＝plan.md 相当。frontmatter に 親計画: backlink
  実装/共通.md           # 実装担当の共通規約（new-plan.shが生成）
  評価/                 # 評価NN・修正NNの置き場（子=NN-子名-評価RR.md／統合=評価RR.md）
  references/           # 任意・遅延：参照資料（完成した材料。考え・未確定は方針/identity.md）
```

1. **親はフォルダ、親ドキュメントは直下 `program.md`。** 子は `plans/` に平置き。
2. **子の状態は program.md の子計画マップが持つ**（リスト形式: 「`- [ ] NN 子計画名 … 状態`」＋ `役割:`／`対象repo:`／`並列:`／`人間ゲート:`／`次:`／`場所:/依存:`／`参照:`。表は使わない）。見出し左のチェックボックスは完了だけ `[x]`、それ以外は `[ ]` とし、plan-opsが状態との不整合を検出する。評価は完了条件を評価MDへ記録することで行い、子に状態バケットは作らない。段階語彙、programの最終一括確認、危険操作の個別承認は `../../AIエージェント基盤/plan-registry/AGENTS.md` に従う。§4.1の英語バケット語彙はフォルダ用で、マップには使わない。
...
テンプレ本文の正本は `../../AIエージェント基盤/skills/plan-ops/templates/`（`plan-v3.md`＝実行ライン方式・既定、`plan.md`＝v2 legacy、`program.md`／子計画／評価／修正に加え、`実行指示.md`・`実行結果.json`・`終了記録.md`）。本節にテンプレ本文を複製しない。規模・評価の規約は `../../AIエージェント基盤/plan-registry/AGENTS.md`、雛形の生成は `skills/plan-ops/scripts/new-plan.sh` / `new-child.sh`（使い方は plan-ops SKILL.md）。

単発 plan.md は `並列:` と `目的/現状/方針/完了条件` を持ち、`形態: program` と子計画マップだけ持たない。program.md は `人間確認方針:`、子計画.md は親計画backlinkに加えて `並列:`／`人間ゲート:` を持つ。programの親正本は `program.md` だけで、同階層に `plan.md` は置かない。
...
3. `active`: 実装・修正・評価中の実行計画（各 `plans/` root あたり最大5件）。
4. `paused`: 一時停止。
...
計画から派生する「実行する作業」は、area 内にフォルダを作らず、指揮官（orca-cockpit）が実装/評価のペインへ直接配って実行する。
