...
近接・注意: 「エージェント定義を作る」Skill。「Skillを作る」skill-creator-custom / skill-creator-codex、「エージェントを運用・指揮する」coding-task-orchestrator / cockpit-supervisor とは別レイヤ。
...
## スキル名: `meta-explain`
概要: メタ構造（hook・Skill・loop・registry・runtime露出・AGENTS・計画構造）の現状と変更計画を、人間が理解しきるまでHTML(Artifact)で説明・反復更新する理解ゲート。本文ドラフト全文までミクロに提示し、対象ファイルは編集しない。
近接・注意: 軽い事前すり合わせは `naiyou-suriawase`、問いで詰める壁打ちは `grill-me`、出力規約は `html`（説明の型はメタ固有分のみ）。旧 `skill-visualizer` の図解・理解用途を吸収（2026-07-11廃止）。
...
近接・注意: デザイン提示・レポートのHTML化は `html`。設定/管理/課金/セキュリティ等のUI設計知識（taxonomy・品質rubric・benchmark・acceptance）を `references/settings/` に内蔵。実装の受け渡し・分割は `coding-task-orchestrator`／`handoff-plan-supervisor` へ。
