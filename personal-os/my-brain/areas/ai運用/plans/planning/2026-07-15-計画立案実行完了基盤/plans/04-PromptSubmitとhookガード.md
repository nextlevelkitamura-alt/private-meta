...
- 依存成果: 02のrun manifest契約（phase語彙）・`bucketctl check`、03のresult packet schema・program-runのレビューキュー仕様
...
7b. **計画日付の陳腐化検知**（2026-07-15人間指示）: session-end系の案内に「このセッションで計画の目的・子構成を大幅更新した場合は、02の `planctl rename` でフォルダ日付を最新化してから終了する」を含める。hookは検知と案内だけを行い、rename自体は実行しない。
