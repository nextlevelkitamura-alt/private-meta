...
ここはrepo-local loopの**実装を置く場所ではない**。所有repoの `loops/` rootへ辿るための
...
implementation-links/
  AGENTS.md
  CLAUDE.md -> AGENTS.md
  <repo-id> -> ../../../../projects/active/<repo>/loops/
```

- 1 repoにつき1本だけ、`<repo-id>` からそのrepoの `loops/` root全体へ相対symlinkを置く。
- 個別loopへのsymlinkは作らない。repo内へ新しいloopを作れば、この入口から自動的に辿れる。
- リンク先の実装・ルールは必ず所有repoの `AGENTS.md` と `loops/AGENTS.md` を正とする。
