...
1. 評価レーンの型を定義: 評価者=codex（gpt-5.5）既定・実装/修正=claude。枠逼迫等で外す場合は「計測実行=claude＋判定=codex後段」の形へ分担を明示し、全体管理者が宣言してから外す（黙って既定を変えない）。SKILL.mdの役割型/レシピに1項で入れる。
...
- [ ] SKILL.md の役割型/レシピに「評価レーン: 評価者=codex既定・外す時は分担明示のうえ全体管理者が宣言」が1項として存在する
