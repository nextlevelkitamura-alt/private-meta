...
`plan-registry` の経路解決（triage決定手続き §6）などが、対象repo・計画箱・規模を解決済みであることを前提にする。ここでは内容を判断せず、雛形生成と**既存ブロック**の機械更新だけを行う。
...
- 既存ファイルは上書きしない。子の目的・完了条件・実行契約は生成後に記入する。評価者は完了条件を評価MDで採点する。
