...
- 実行形: delegated-parallel（Aレーン=既存計画の監査＝read-onlyでreferencesへ出力・**worktree不要**、Bレーン=pilot＝identity統合・知識分類・代表移行・**task-scoped worktree**。ファイル非交差）
...
- [x] 監査一覧が全バケット・全計画を対象に、件数/上限・実装根拠・最終評価・人間確認・program子状態・終了記録の有無を含み、各行が元ファイルへのパスを持つ。
- [x] 監査結果が「確定事実」と「人間判断が必要な候補」を分け、承認なしに計画をarchive/done/pausedへ移していない。候補（並列実装フローのmerged closeを含む）が推奨付きで承認セットに記録されている。
- [x] `ai運用/AGENTS.md` にidentity.md全項目の統合先があり対応表で網羅を確認できる。identity.md削除・知識/全ファイルの分類が承認セットに載っており、承認前の削除・移動が無い。
- [x] 代表の単発plan 1件・program 1件が新テンプレへ移行され、plan-lint・program-lintが通る。
- [x] `areas/AGENTS.md` §1 が新Area標準を示し、他areaへの適用が「pilot合格後」と明記され、work/money/healthに変更が無い。
- [x] 旧pathを参照する箇所のgrep結果（移動実施前なら対象参照の一覧）が承認セットに添付されている。

## 実装結果

- 2026-07-16 評価02で全PASS（A=監査67計画網羅・増補c8edeee／B=pilot task/pf05b=ebf8f43・統合branchへmerge済み）。実行はせず候補一覧＝承認セット行き（identity.md削除候補・知識/維持2・監査是正候補・決定ログ#18矛盾1件）。
