...
program.md / plan.md の見え方を標準化し、①目的 ②並列化の有無 ③レビュー方式(都度/一括) ④進捗チェックボックス を計画に持たせる。あわせて active の同時実行数を制御(WIP)し、planning→active の昇格を機械化する。「見て進捗が分かる・active が溢れない・移動が手作業で漏れない」状態にする。

## 全体像

plan-ops(実装正本 `personal-os/AIエージェント基盤/skills/plan-ops/`)のテンプレ・スクリプト・lint と、規約正本(`areas/AGENTS.md` §3・`説明書/運用契約.md` §2)を横断で改定する。見え方(01)→機械追従(02)は直列、レビュー方式(03)・WIP(04)は独立、昇格自動化(05)は04依存。この program.md 自身を新フォーマットの実例にする(左チェックボックス列は 02 のパーサ対応後に全 program へ一斉投入。先に付けると program-lint が0件検出で誤合格するため、今はヘッダを従来形で保つ)。
...
03  レビュー方式(都度/一括) … 計画
    ...
    場所: plans/03 ／ 依存: ―
04  WIP3とactive再定義 … 計画
    並列: 可 ／ レビュー: 都度(移行32件)
    次: active=実行中のみ≤3・育成中→planning/paused を §3 に定義、ai運用 32→3 をクリーンアップ
    場所: plans/04 ／ 依存: ―
05  バケット遷移の自動化(bucketctl) … 計画
    並列: 不可 ／ レビュー: 都度(script＋テスト)
    次: planning→active昇格＋WIP上限で弾く script を作り kickoff/plan-triage から呼ぶ(追い出し先の選択は人間)
    場所: plans/05 ／ 依存: 04
...
- [ ] planning→active 昇格 script が WIP上限超過を弾き、kickoff/plan-triage から呼ばれる導線がある。
