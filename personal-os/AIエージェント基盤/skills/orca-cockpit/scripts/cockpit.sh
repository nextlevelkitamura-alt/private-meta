...
# 段階イベントJSONL（up/send/down・追記のみ・git非管理）。置き場・スキーマ・ローテーション方針は
# loops-registry/loops/renderer/loop.md に明記（レンダラのbuild-board-now.shが同じ既定パスを読む）。
...
# ローテーション方針は events.jsonl と同じ流儀（loop.md準拠）。上書きは環境変数。
...
後方互換・無指定はnull)。上書きは環境変数COCKPIT_EVENTS_FILE。置き場・スキーマ・ローテーション方針は
loops-registry/loops/renderer/loop.md。
