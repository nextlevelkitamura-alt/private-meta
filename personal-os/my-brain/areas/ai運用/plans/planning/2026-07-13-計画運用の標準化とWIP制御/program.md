...
plan-ops(実装正本 `personal-os/AIエージェント基盤/skills/plan-ops/`)のテンプレ・スクリプト・lint と、規約正本(`areas/AGENTS.md` §3・`説明書/運用契約.md` §2)を横断で改定する。見え方(01)→機械追従(02)は直列、レビュー方式(03)・WIP＋昇格自動化(04)は独立。この program.md 自身を新フォーマットの実例にする(左チェックボックス列は 02 のパーサ対応後に全 program へ一斉投入。先に付けると program-lint が0件検出で誤合格するため、今はヘッダを従来形で保つ)。
...
03  レビュー方式(都度/一括) … 計画
    ...
    場所: plans/03 ／ 依存: ―
04  WIP3とactive再定義(＋bucketctl昇格自動化) … 計画
    並列: 可 ／ レビュー: 都度(移行32件＋script)
    次: active=実行中のみ≤3・育成中→planning/paused を §3 に定義、bucketctl で planning→active 昇格を自動化(上限で弾く)、ai運用 32→3 をクリーンアップ
    場所: plans/04 ／ 依存: ―
