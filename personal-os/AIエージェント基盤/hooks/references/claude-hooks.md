...
## 4. hook の型（5種・うち2種を session-board で使用）

- `command`: シェル/スクリプトを実行。stdin に JSON を受ける。【session-board使用: 受け口 `.py` 3本】
- `prompt`: 単一ターンでモデルに判定させる（yes/no）。fast-model で走る。【session-board使用: `milestone.md`】
...
session-board の `milestone.md` がこれ。返す JSON は `{"ok":true}`（停止してよい）／ `{"ok":false,"reason":"..."}`（reason を注入して継続）。**`ok` 形式が正**で、`decision` 形式ではない（ローカル確認）。
...
その上に §8・§3 の便利機能（prompt型 milestone・agent別hook・worktree隔離・豊富な frontmatter）を上乗せする。
...
session-board の登録スニペットは `../session-board/README.md`。session-board 関連の Claude 登録は包括承認済み（承認ルールB）。ただし基盤の規律どおり、session-board 以外の hook の user 設定への登録は人間ゲート。
