...
- **`--settings`**: 役割別許可の正本 `settings/inbox-triage-permissions.json`。読み=広く（Read/Glob/Grep）、書き（Write/Editツール）=`Private/**/plans/planning/**` とデイリー `ゴール/デイリー/**`（処理印の行の置換）のみ。Bashは `ls` と `scripts/new-plan-planning.sh`（--out を planning 配下へ強制するラッパ）だけをallow。素の new-plan.sh・mkdir は敵対的レビュー（2026-07-09・planning外へ書ける穴の実証）を受けてallowから撤去。
...
- インボックス見出し・マーカー流儀の正本: `my-brain/ゴール/templates/デイリー.md`（8節・2026-07-09に正本救出。旧 renderer/templates 側は移設済みポインタ）。節見出しの凍結文言は「デイリー運用刷新」program の実装契約§1。
