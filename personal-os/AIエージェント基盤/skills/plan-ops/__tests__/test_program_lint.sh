...
rmdir "$ROLE_DIR/評価"
evaluation_out="$("$SCRIPTS/program-lint.sh" "$ROLE_DIR/program.md")"
evaluation_rc=$?
assert_eq "(g) 評価/無しはexit 1" "$evaluation_rc" "1"
assert_contains "(g) 評価/欠落を指摘" "$evaluation_out" "評価/ フォルダが無い"
mkdir -p "$ROLE_DIR/評価"

printf '# 旧親plan\n' > "$ROLE_DIR/plan.md"
mkdir -p "$ROLE_DIR/レビュー"
printf '\n    レビュー: 都度\n' >> "$ROLE_DIR/program.md"
legacy_out="$("$SCRIPTS/program-lint.sh" "$ROLE_DIR/program.md")"
legacy_rc=$?
assert_eq "(h) 旧親plan・レビュー構造はexit 1" "$legacy_rc" "1"
assert_contains "(h) sibling plan.mdを指摘" "$legacy_out" "併存plan.mdは置けない"
assert_contains "(h) レビュー/を指摘" "$legacy_out" "レビュー/ フォルダは使えない"
assert_contains "(h) レビューfieldを指摘" "$legacy_out" "旧「レビュー:」フィールドは使えない"
