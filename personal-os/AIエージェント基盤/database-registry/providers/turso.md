...
3. 認証有無を確認する必要がある時だけ`turso db list`をローカル端末で実行し、終了状態だけを確認する。database一覧、認証先、出力は表示・記録・commitしない。
4. 未認証時だけ`turso auth login`を対話的に実行する。出力、認証先、資源一覧を記録しない。
5. 実データベースの確認はDashboardで行う。変更系の`create`、`destroy`、token生成はこの案内から実行しない。

database作成、location変更、migration、token生成の前には、対象database、環境、権限、readback、rollbackを対象repoの実行契約で確定する。
