...
**テーマは計画ではない**。目的（なぜ・どこまで）を育てる場所であり、**上限なし・評価なし・停滞という概念なし**。`thinking/` 廃止後に未成熟な考えが planning へ流れ込んでいた受け皿を、ここへ移す（2026-07-24 新設）。揺れている構想・アイデア・壁打ちの積み重ねはここで育て、実行を決めた時だけ planning へ計画を切り出す。

Themeが固有の実行計画を持つ場合は、最寄り `AGENTS.md` が計画箱を宣言する。構想の `concepts/` と実行の `plans/` を分け、横断計画はarea直下の `plans/` に残す。`計画システム再設計` はこの方式を採用し、構想本文はローカル/Git正本のまま、将来Tursoへ移す候補の短い状態・活動履歴とMarkdown本文を二重保存しない。

```text
areas/<area>/
  themes/
    <テーマslug>/
      theme.md         # 下記テンプレ（6節固定・すべて追記型で育てる）
      壁打ち/
        YYYY-MM-DD.md  # 壁打ちセッションごとの追記ログ（日付ファイル）
    _closed/<slug>/    # 閉じたテーマ（達成/取り下げの区分と一行理由を theme.md 末尾に残す）
    _light/<名前>.md   # ライトテーマ（割り込み・逸れ物の受け皿。1ファイル・下記）
...
親ゴール: 年間#<slug>（`../ゴール` の的。無ければ「未接続」と明記し、月間計画に的のドラフトを提案）
board-theme-id: <ID>（board DB との照合キー。名前は DB 側が正・改名に耐える）

## 目的（なぜやるか。1〜3行）
## 完成形スケッチ（使用体験。誰が・どう使い・何がどう変わるかのシナリオ。ぼんやりなら「未確定」と書いて壁打ちで育てる）
## ゴール（達成されたと言える状態。3〜7個、検証可能な条件で。1行で「完成」できる浅い的にしない）
## 現在の考え（未確定のまま書く。壁打ちのたびに更新）
## 派生計画（plans/ へ切り出したものへのリンクと現状）
```

- **親ゴールで縦ladderと内容を接続する**: テーマが「的の内容（なぜ・どこまで）」を運ぶ器になり、ゴール階層（`../ゴール`）→ テーマ → 計画 → 実行 が内容で繋がる。slug の形式だけで内容が渡らなかった断絶をここで埋める。
- **完成形スケッチが目標の浅さ対策**: 的が1行で「簡単に完成」してしまう現状への直接の解。この節が「未確定」のままでは実行（後述の /goal）に入らない。
- **壁打ちを揮発させない**: grill-me / naiyou-suriawase / 通常チャットの壁打ち結果は必ず `壁打ち/YYYY-MM-DD.md` に追記し、確定した分だけ theme.md の「現在の考え」へ昇格させる。

**Focusmap Theme とのハイブリッド共存（正本分担・2026-07-24）**:

- **名前・当日の実行状態 = board DB（Turso）が正本**。Web UI 上でテーマ名が変更されうるため、AI はセッション開始時に毎回 DB から取得して考える。
- **目的・完成形スケッチ・ゴール（完了条件）・壁打ちログ・計画資料 = ローカル md が正本**。コンテキストの本体はこちらで、**完了判定は theme.md のゴールで行う**。本文は DB へ送らない。
...
**ライトテーマ（`themes/_light/<名前>.md`・割り込みと逸れ物の受け皿）**:

- テーマ作業中に出る「今すぐやること」「ふと思いついた修正」は、手を止めずまず**既存ライトテーマと照合**し、合えばその配下に todo として入れ子登録する。合致が無ければ**親目的1行だけのライトテーマを自動起案**する。
- ライトテーマは恒常的な軽い受け皿（例: LINE返信、ちょい修正、経理まわり）。定義は1ファイル（親目的1〜3行＋照合キーワード）に留め、完成形スケッチや評価は持たない。
- **セッションも入れ子で束ねる**: 未紐付けセッション（例: LINE返信セッション群）をライトテーマ配下にネスト表示する。未照合のセッションだけが未紐付け欄に残る。
- 同類の todo が3件溜まったライトテーマは、通常テーマへの昇格（theme.md 化）を提案する。

**テーマ連動クローズ**: テーマを閉じる（`_closed/` へ）時、派生計画に終了区分（completed/superseded/cancelled…）を付けて一括で閉じる。「目的が消えたのに計画だけ生きている」状態を構造的に作れなくする。移動・クローズは §3.3 のバケット規約と人間確認に従う。
