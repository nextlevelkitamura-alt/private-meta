...
- [x] 01 ボード純UI … 修正02まで実装済み（本番実機の人間確認待ち）
    役割: 実装
...
- [x] 02 計画接続 … 実装完了（テスト603PASS・inbox migration適用済み・push済み・本番実機確認待ち）
    役割: 実装
    ...
    場所: plans/02 ／ 依存: 子01
- [x] 03 サブエージェント詳細化 … 捕捉側統合＋表示側は子06のAIレーンへ吸収（sub-detail=SessionSubagent detail5列表示・ライブ捕捉2026-07-21実証）
    役割: 実装
...
- [x] 05 計画直結ボード … 計画（2026-07-21人間の方向転換=カード軸をテーマから計画へ・シェルtodo畳み・active計画の常時表示）
    役割: 実装
    対象repo: focusmap
...
- [x] 06 理想形モック駆動の仕上げ … v6実装完了・push済み・本番デプロイ中（実機確認待ち）
    役割: 実装
    ...
    場所: plans/06 ／ 依存: 子05
- [x] 07 テーマ上位4段化 … v6の上にテーマ層を追加(案A・2026-07-22人間指摘=テーマは複数プログラムを束ねる上位)。実装完了・push済み・本番デプロイ中
    役割: 実装
    ...
    場所: plans/07 ／ 依存: 子06
- [x] 04 完了移動の運用実測 … 2026-07-21〜22の実運用で消化（実機使用中に修正01・子07の課題を検出し即修正＝完了移動と運用の実測を兼ねた）
    役割: 評価
    対象repo: なし（観測と記録）
    並列: 不可（子01〜03完了後）
    人間ゲート: なし
...
- [x] 全子（01〜07）の完了条件がコードレベルでPASS。統合評価は `評価/まとめ評価01.md`（修正02+子05+子06一括・APPROVED）＋各修正md
- [x] 統合評価 `評価/まとめ評価01.md` が全PASS（FAIL0・憲法4条違反0）。子07/修正01は同系列の追加実装で回帰なし
- [x] 本番実機で「テーマ→計画→工程→AIレーン（4段）＋計画チップ＋サブ詳細＋📄」が実データで動作・人間OK（2026-07-22）

※ 状態の正本はフォルダ。2026-07-22 人間クローズ判断で active→done（終了記録.md 参照）。当初モックv2（テーマ唯一軸）は実機フィードバックで v6=3段ドリルダウン→案A=テーマ上位4段へ発展した。
