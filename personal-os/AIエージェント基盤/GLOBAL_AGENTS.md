...
- 依頼の目的と対象範囲が明確なら、AIはファイル作成・編集・設定変更・DB反映・検証・git仕上げまで自律実行する。対象、目的、影響は進行中または完了報告で明示し、事前確認のためだけに停止しない。
...
## 5. 安全ルール
- 削除、移動、改名、正本変更、履歴操作は、正確な対象・依存参照・回復手段を先に解決してからAIが実行する。対象が曖昧、範囲が過大、回復不能、または依頼に必要な権限が無い時だけ停止して確認する。
...
- サクッとは「変更1〜2ファイル・容易に戻せる・依頼の目的と完了条件が明確」の全てを満たす時だけ、計画書なしで実行し事後報告する。1つでも外れたらライト以上として計画を置く。
- hook/launchd登録、commit、通常push、main反映、外部公開、本番データ反映、additiveなDB migrationは、依頼範囲内ならAIが事前検証・実行・readback・rollback確認まで進める。別の業務データ、金銭、対人送信、credential変更など新しい権限を要する操作は、その権限が明示されていない時だけ確認する。
- 認証、質問、waiting、利用上限は担当AI/指揮官がまず解消し、AIだけでは取得できない認証または結果を変える未確定の人間判断だけを上げる。
- headlessは定期実行かつ人が完了を待たない仕事だけに使う。人が待つ実装・評価は、状態が見える実行経路で動かす。キュー・headless・hookの方式定義は `loops-registry/AGENTS.md` を正とする。
- 計画状態はフォルダだけで持つ。`done` は最終評価md全PASS済み、`archive` は終了記録を残した閉じた参照専用計画である。未解決の人間判断が無ければAIが検証後に遷移を完了する。遷移・容量・終了区分の機械検証は plan-ops の `bucketctl` / `planctl` を使い、状態台帳を追加しない。
