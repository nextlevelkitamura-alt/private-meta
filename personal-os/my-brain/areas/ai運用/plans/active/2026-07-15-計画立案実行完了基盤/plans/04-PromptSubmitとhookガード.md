...
- [ ] 初回・ミラー注入の**候補文**（common.pyが生成）に planning→active→done→archive の責務分離、容量案内、レビュー方式（一括/都度）の1行、`bucketctl check` への誘導があり、旧 `done=未評価` や自動archiveを示す文言が残っていないことをテストで検証できる。本文の生成元はcommon.pyのみで、runtime別シムに本文コピーを増やしていない。候補文の実注入への接続（有効化）は人間承認後の適用ラウンドで行い、**この子の完走ラインは候補文＋テスト＋登録差分まで**（実行契約の未有効化と両立させる。評価01の指摘で明確化・2026-07-15）。
- [ ] サクッと3条件が全YESでない、または不明な入力を `plan-management` へ案内し、hookがrepo・計画箱・レビュー合否・バケット遷移を決めないことを、**候補文とそのテスト**で確認できる（実注入での確認は承認後の適用ラウンドで再評価）。
...
- [ ] SubagentStartが implementerの割当ずれ（worktree_path・branch・base_commitの不一致）を編集開始前に検出し、read-only roleでは省略する。SubagentStopが implementerのresult欠落・reviewerの評価欠落を検出する。どちらもCodex/Claude両方のstdin fixtureで検証でき、ガードは計画本文・チェックボックス・バケット・worktreeを一切変更しない（実行前後の状態不変テスト）。
